...
const getHandler = async () => {
    ...
    try {
        importedModule = await import(`${LAMBDA_TASK_ROOT}/${moduleToImport}.js`)
    } catch (e) {
...
const patchIO = async (method, payload) => {
    const warning = `
...
// Lambda layer do not support top level await yet. This init function exists to be able to await import newreliec.
// The handler will be called for each request to the lambda but the init function should be called only the first time.
const init = async () => {
    if (!initialized) {
        newrelic = await import('newrelic')
        console.log(newrelic);
        wrappedHandler = newrelic.setLambdaHandler(await getHandler())
        initialized = true;
    }
}
